...
In all data frames, `speed` 0 identifies the sitting rest phase, `speed` 1 identifies standing rest phase, and all other `speed` values identify walking speed in km^.^h^-1^.

Cardiorespiratory data came from indirect calorimetry with data collected breath-by-breath and averaged over 5 sec epochs. Accelerometer metrics used were activity counts (AC), mean amplitude deviation (MAD) [[@vaha15]](#refs) and euclidean norm minus one (ENMO) [[@vhees13]](#refs). All metrics were computed based on resultant vector data. Primary accelerometer was only used to obtain AC, as it can be only obtained from primary accelerometer data and computed according to proprietary procedures [[@act12]](#refs). Secondary accelerometer was used to obtain raw acceleration data and to compute MAD and ENMO, because, unlike the primary accelerometer, it does not apply any filtering on data. AC were computed with ActiLife software (version 6.13.3),  MAD and ENMO were computed with `GGIR` [package](https://cran.r-project.org/web/packages/GGIR/index.html). All accelerometer metrics values on the data frames are averages of 5 sec epochs of each protocol period.

Subsequent data analyses for all parameters were conducted with the penultimate 30 sec of sitting position, standing position and all walking speeds, ensuring the steady state of VO2
...
Finally, `cardio` and `..._acc` data frames were merged to obtain a single data frame to each accelerometer placement. Key variables used to connect each pair of data frames were `ID` and `speed`. A key is a variable (or set of variables) that uniquely identifies an observation [[@R4DS]](#refs). 
...
hip <- hip_pri_acc %>% 
  ...
  select(ID, speed, AC, MAD, ENMO, kcal, VO2.kg, MET, HR, everything())
...
Agreement between kilocalories computed from data obtained from indirect calorimetry and predicted kilocalories was assessed by Bland-Altman plots [[@ba86; @giav15]](#refs). The difference of the measured and predicted kilocalories was plotted against their mean. The mean of these differences represented the bias and this mean was also used to calculate the limits of agreement with 95% confidence intervals (LoA). 
