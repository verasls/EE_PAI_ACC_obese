...
Accelerometer metrics used were activity counts (AC), mean amplitude deviation (MAD) [[@vaha15]](#refs) and euclidean norm minus one (ENMO) [[@vhees13]](#refs). All metrics were computed based on resultant vector data. Primary accelerometer was only used to obtain AC, as it can be only obtained from primary accelerometer data and computed according to proprietary procedures [[@act12]](#refs). Secondary accelerometer was used to obtain raw acceleration data and to compute MAD and ENMO, because, unlike the primary accelerometer, it does not apply any filtering on data. All accelerometer metrics values on the data frames are averages of 5 sec epochs of each protocol period.
...
Some computations were made to obtain other variables using user-defined functions. `get_MET` function **(insert link)** computes metabolic equivalents (MET) based on the sitting rest phase oxygen uptake (VO~2~). `get_PAI_categories` function **(insert link)** computes sedentary activity (SA) and physical activity intensities (PAI) categories based on MET values. Activities were classified as: ≤ 1.5 MET – SA; 1.6–2.9 MET – light physical activity (LPA);  3.0–6.0 MET – moderate physical activity (MPA); and >5.5 MET – vigorous physical activity (VPA) [[@garb11]](#refs). These variables were also recoded to create ordinal variables, according to the following identification: 1 – SA, 2 – LPA, 3 – MPA and 4 – VPA. `get_HR_res` function **(insert link)** computes percentages of heart rate reserve and `get_kcal` function **(insert link)** computes kilocalories values based on VO~2~ and carbon dioxide production (VCO~2~) [[@weir49]](#refs).
...
descriptives <- summarise(
  .data = samp_desc,
  age_mean    = myround(mean(age), digits = 1),
  age_sd      = myround(sd(age), digits = 1),
  weight_mean = myround(mean(weight_kg), digits = 1),
  weight_sd   = myround(sd(weight_kg), digits = 1),
  height_mean = myround(mean(height_cm), digits = 1),
  height_sd   = myround(sd(height_cm), digits = 1),
  BMI_mean    = myround(mean(BMI_kgm2), digits = 1),
  BMI_sd      = myround(sd(BMI_kgm2), digits = 1),
  fat_mean    = myround(mean(body_fat), digits = 1),
  fat_sd      = myround(sd(body_fat), digits = 1)
)
