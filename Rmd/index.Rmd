...
`get_kcal` function **(insert link)** computes EE (in kilocalories) with Weir's equation, based on VO~2~ and VCO~2~ [[@weir49]](#refs). This variable will be used to develop regression equations in the [**Linear mixed models**](#LMM) section. `get_MET` function **(insert link)** computes metabolic equivalents (MET) based on the sitting rest period VO~2~. `get_PAI_categories` function **(insert link)** computes SA and PAI categories based on MET values. Activities were classified as: ≤ 1.5 MET – SA; 1.6–2.9 MET – light physical activity (LPA);  3.0–6.0 MET – moderate physical activity (MPA); and >5.5 MET – vigorous physical activity (VPA) [[@garb11]](#refs). These variables were also recoded to create ordinal variables, according to the following identification: 1 – SA, 2 – LPA, 3 – MPA and 4 – VPA. PAI categories variables will be used to develop cut-points to classify SA and PAI in the [**Receiver operating characteristic curves**](#ROC) section.
...
LOOCV of the LMM was done with the `cross_validate_mixed_model` function **insert link** and is shown below.
...
LOOCV of the ROC curves was done with the `cross_validate_ROC_curves` function **insert link** and is shown below.
...
The accuracy of predicted EE was assessed by mean absolute error (MAE), mean absolute percent error (MAPE), and root mean square error (RMSE). These indices were computed with the `accuracy_indices` function **insert link**.
