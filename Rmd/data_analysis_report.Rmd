...
First, `cardio`, `hip_pri_acc`, `hip_sec_acc`, `back_pri_acc` and `back_sec_acc` data frames were loaded. `cardio` data frame contained the following variables: subject identification number (ID), speed (identify sitting and standing positions and walking speeds), sex (M, male; F, female), age (years), body mass (kg). body mass index (BMI, kg^.^m^-2^), oxygen uptake (VO~2~, ml^.^min^-1^), carbon dioxide production (VCO~2~, ml^.^min^-1^) and relative VO~2~ (ml^.^kg^-1^min^-1^).  `hip_pri_acc` and `back_pri_acc` data frames contained activity counts (AC) values from primary accelerometer data for hip and back accelerometer placement, respectively. `hip_sec_acc`, and `back_sec_acc` data frames contained mean amplitude deviation (MAD) [[@vaha15]](#refs) and euclidean norm minus one (ENMO) [[@vhees13]](#refs) values from secundary accelerometer data for hip and back accelerometer placement, respectively. All `..._acc` data frames also contained ID and speed variables. In all data frames, speed 0 identifies the sitting rest period, speed 1 identifies standing rest period, and all other speed values identify walking speed in km^.^h^-1^.
...
Then, some computations needed to be made in the `cardio` data frame to obtain other variables that would be necessary in subsequent data analysis. All these computations were done using user-defined functions.

`get_kcal` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/get_kcal.R) computes EE (in kilocalories) with Weir's equation, based on VO~2~ and VCO~2~ [[@weir49]](#refs). This variable will be used to develop regression equations in the [**Linear mixed models**](#LMM) section. `get_MET` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/get_MET.R) computes metabolic equivalents (MET) based on the sitting rest period VO~2~. `get_PAI_categories` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/get_PAI_categories.R) computes SA and PAI categories based on MET values. Activities were classified as: ≤ 1.5 MET – SA; 1.6–2.9 MET – light physical activity (LPA);  3.0–6.0 MET – moderate physical activity (MPA); and >5.5 MET – vigorous physical activity (VPA) [[@garb11]](#refs). These variables were also recoded to create ordinal variables, according to the following identification: 1 – SA, 2 – LPA, 3 – MPA and 4 – VPA. PAI categories variables will be used to develop cut-points to classify SA and PAI in the [**Receiver operating characteristic curves**](#ROC) section.
...
Finally, `cardio`, `hip_pri_acc`, `hip_sec_acc`, `back_pri_acc` and `back_sec_acc` data frames were merged to obtain a single data frame to each accelerometer placement (`hip` and `back` data frames). Key variables used to connect each pair of data frames were ID and speed. A key is a variable (or set of variables) that uniquely identifies an observation [[@R4DS]](#refs).
...
summary(back_ENMO_model)
r2_back_ENMO_model
```

```{r R2_range, echo=FALSE}
minR2 <- myround(min(c(r2_hip_AC_model[[6]], r2_hip_MAD_model[[6]], r2_hip_ENMO_model[[6]], r2_back_AC_model[[6]], r2_back_MAD_model[[6]], r2_back_ENMO_model[[6]])), digits = 2)
maxR2 <- myround(max(c(r2_hip_AC_model[[6]], r2_hip_MAD_model[[6]], r2_hip_ENMO_model[[6]], r2_back_AC_model[[6]], r2_back_MAD_model[[6]], r2_back_ENMO_model[[6]])), digits = 2)
```
...
Cut-points that identify SA and PAI created from AC, MAD and ENMO were obtained applying receiver operating characteristic curves (ROC) [[@faw06; @jago07]](#refs) for both hip and back accelerometer placement data. The indices used to summarize the cut-points were sensitivity (proportion of true positives that are correctly identified), specificity (proportion of false positive that are correctly identified) and the area under the curve. RMR were not used in the ROC analysis. LPA boundaries were provided with SA and MPA  cut-points. SA and PAI variables were created with `get_PAI_categories` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/get_PAI_categories.R) as described [here](#prep_data).ROC curves were created using `roc` function of the `pROC` [package](https://cran.r-project.org/web/packages/pROC/pROC.pdf). Optimal cut-points were chosen using the closest-to-(0,1) criterion with `coords` function, also of the `pROC` [package](https://cran.r-project.org/web/packages/pROC/pROC.pdf) (`coords(roc, x = "best", best.method = "closest.topleft")`) [[@perk06]](#refs).
...
cp_hip_AC_ROC_SED <- coords(hip_AC_ROC_SED, x = 13)
...
cp_hip_MAD_ROC_SED <- coords(hip_MAD_ROC_SED, x = 8)
...
cp_hip_ENMO_ROC_SED <- coords(hip_ENMO_ROC_SED, x = 37)
...
cp_back_AC_ROC_SED <- coords(back_AC_ROC_SED, x = 10)
...
cp_back_MAD_ROC_SED <- coords(back_MAD_ROC_SED, x = 7)
...
cp_back_ENMO_ROC_SED <- coords(back_ENMO_ROC_SED, x = 34)
...
Summary statistics for all ROC curves can be found in table below.

```{r ROC_summary}
ROC_summary <- data.frame(
  acc_placement = c(rep("Hip", 9), rep("Back", 9)),
  acc_metric    = rep(c(rep("AC", 3), rep("MAD", 3), rep("ENMO", 3)), 2),
  intensity     = rep(c("Sedentary", "Moderate", "Vigorous"), 6),
  threshold     = c(
    round(cp_hip_AC_ROC_SED[[1]], 0), round(cp_hip_AC_ROC_MOD[[1]], 0), round(cp_hip_AC_ROC_VIG[[1]], 0),
    round(cp_hip_MAD_ROC_SED[[1]], 0), round(cp_hip_MAD_ROC_MOD[[1]], 0), round(cp_hip_MAD_ROC_VIG[[1]], 0),
    round(cp_hip_ENMO_ROC_SED[[1]], 0), round(cp_hip_ENMO_ROC_MOD[[1]], 0), round(cp_hip_ENMO_ROC_VIG[[1]], 0),
    round(cp_back_AC_ROC_SED[[1]], 0), round(cp_back_AC_ROC_MOD[[1]], 0), round(cp_back_AC_ROC_VIG[[1]], 0),
    round(cp_back_MAD_ROC_SED[[1]], 0), round(cp_back_MAD_ROC_MOD[[1]], 0), round(cp_back_MAD_ROC_VIG[[1]], 0),
    round(cp_back_ENMO_ROC_SED[[1]], 0), round(cp_back_ENMO_ROC_MOD[[1]], 0), round(cp_back_ENMO_ROC_VIG[[1]], 0)
  ),
  sensitivity   = c(
    myround(cp_hip_AC_ROC_SED[[3]], 2), myround(cp_hip_AC_ROC_MOD[[3]], 2), myround(cp_hip_AC_ROC_VIG[[3]], 2),
    myround(cp_hip_MAD_ROC_SED[[3]], 2), myround(cp_hip_MAD_ROC_MOD[[3]], 2), myround(cp_hip_MAD_ROC_VIG[[3]], 2),
    myround(cp_hip_ENMO_ROC_SED[[3]], 2), myround(cp_hip_ENMO_ROC_MOD[[3]], 2), myround(cp_hip_ENMO_ROC_VIG[[3]], 2),
    myround(cp_back_AC_ROC_SED[[3]], 2), myround(cp_back_AC_ROC_MOD[[3]], 2), myround(cp_back_AC_ROC_VIG[[3]], 2),
    myround(cp_back_MAD_ROC_SED[[3]], 2), myround(cp_back_MAD_ROC_MOD[[3]], 2), myround(cp_back_MAD_ROC_VIG[[3]], 2),
    myround(cp_back_ENMO_ROC_SED[[3]], 2), myround(cp_back_ENMO_ROC_MOD[[3]], 2), myround(cp_back_ENMO_ROC_VIG[[3]], 2)
  ),
  specificity   = c(
    myround(cp_hip_AC_ROC_SED[[2]], 2), myround(cp_hip_AC_ROC_MOD[[2]], 2), myround(cp_hip_AC_ROC_VIG[[2]], 2),
    myround(cp_hip_MAD_ROC_SED[[2]], 2), myround(cp_hip_MAD_ROC_MOD[[2]], 2), myround(cp_hip_MAD_ROC_VIG[[2]], 2),
    myround(cp_hip_ENMO_ROC_SED[[2]], 2), myround(cp_hip_ENMO_ROC_MOD[[2]], 2), myround(cp_hip_ENMO_ROC_VIG[[2]], 2),
    myround(cp_back_AC_ROC_SED[[2]], 2), myround(cp_back_AC_ROC_MOD[[2]], 2), myround(cp_back_AC_ROC_VIG[[2]], 2),
    myround(cp_back_MAD_ROC_SED[[2]], 2), myround(cp_back_MAD_ROC_MOD[[2]], 2), myround(cp_back_MAD_ROC_VIG[[2]], 2),
    myround(cp_back_ENMO_ROC_SED[[2]], 2), myround(cp_back_ENMO_ROC_MOD[[2]], 2), myround(cp_back_ENMO_ROC_VIG[[2]], 2)
  ),
  AUC           = c(
    myround(auc(hip_AC_ROC_SED)[[1]], 2), myround(auc(hip_AC_ROC_MOD)[[1]], 2), myround(auc(hip_AC_ROC_VIG)[[1]], 2),
    myround(auc(hip_MAD_ROC_SED)[[1]], 2), myround(auc(hip_MAD_ROC_MOD)[[1]], 2), myround(auc(hip_MAD_ROC_VIG)[[1]], 2),
    myround(auc(hip_ENMO_ROC_SED)[[1]], 2), myround(auc(hip_ENMO_ROC_MOD)[[1]], 2), myround(auc(hip_ENMO_ROC_VIG)[[1]], 2),
    myround(auc(back_AC_ROC_SED)[[1]], 2), myround(auc(back_AC_ROC_MOD)[[1]], 2), myround(auc(back_AC_ROC_VIG)[[1]], 2),
    myround(auc(back_MAD_ROC_SED)[[1]], 2), myround(auc(back_MAD_ROC_MOD)[[1]], 2), myround(auc(back_MAD_ROC_VIG)[[1]], 2),
    myround(auc(back_ENMO_ROC_SED)[[1]], 2), myround(auc(back_ENMO_ROC_MOD)[[1]], 2), myround(auc(back_ENMO_ROC_VIG)[[1]], 2)
  )
)
kable(ROC_summary, align = c("l", "l", "l", "c", "c", "c", "c"), col.names = c("Accelerometer Placement", "Accelerometer Metric", "Intensity Category", "Threshold", "Sensitivity", "Specificity", "AUC"))
```


# Validation analysis

The validity of equations and cut-points developed were posteriorly analyzed through leave-one-out cross-validation (LOOCV) method [[@staud12]](#refs). Hence, each participant’s data was separated into a testing dataset (one at a time) with the remaining data being in the training dataset. New LMM and ROC curves, with the same structure as determined for the entire sample, were developed using the training dataset to predict kilocalories and identify SA or PAI for the only participant in the testing dataset. This process was repeated for all participants (`r nrow(samp_desc)` times).  

LOOCV of the LMM was done with the `cross_validate_mixed_model` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/cross_validate_mixed_model.R).
...
LOOCV of the ROC curves was done with the `cross_validate_ROC_curves` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/cross_validate_ROC_curves.R).
...
Agreement between measured and predicted EE was assessed by Bland-Altman plots [[@ba86; @giav15]](#refs). The difference of the measured and predicted EE was plotted against their mean. The mean of these differences represented the bias and this mean was also used to calculate the limits of agreement with 95% confidence intervals (LoA). 
...
Code to generate these plots can be seen [here](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/figs/fig1.R).

<br>

Linear regressions were applied to identify if there was any proportional bias, that is, if bias was related with the magnitude of the mean between measured and predicted EE. Linear regressions were run using the `lm` function of the base R. No proportional bias was identified (*p*<0.05).
...
The accuracy of predicted EE was assessed by mean absolute error (MAE), mean absolute percent error (MAPE), and root mean square error (RMSE). These indices were computed with the `accuracy_indices` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/accuracy_indices.R).
...
kable(accuracy, col.names = c("Accelerometer Placement", "Accelerometer Metric", "MAE", "MAPE", "RMSE"))
```

All conditions showed a MAE near `r myround(mean(accuracy$MAE), digits = 1)` kcal, MAPE from near `r round(min(accuracy$MAPE), digits = 0)`% to near  `r round(max(accuracy$MAPE), digits = 0)`% and RMSE of approximately `r myround(mean(accuracy$RMSE), digits = 2)` kcal.

## Kappa statistic

Kappa statistic (κ) was used to measure the classification agreement of SA and PAI obtained from indirect calorimetry and those obtained from cut-points [[@watson10]](#refs). Individual classification agreement analyses for SA, LPA, MPA and VPA were done with unweighted Kappa method [[@sim05]](#refs). Because the whole classification included four ordinal categories, the disagreement magnitude among them (e.g., misclassified by two or more categories instead of one) was also analyzed in a global classification agreement utilizing a quadratic weighted Kappa method [[@sim05]](#refs). 

To perform unweighted Kappa analyses, METs obtained from indirect calorimetry and values of AC, MAD, ENMO included in cut-points of SA, LPA, MPA and VPA were recoded to create binary variables (0 or 1), wherein 1 was the activity to be identified and 0 the others. For quadratic weighted Kappa analyses, METs obtained from indirect calorimetry and values of AC, MAD and ENMO included in cut-points for SA, LPA, MPA and VPA were recoded to create ordinal variables, according to the following identification: 1 – SA, 2 – LPA, 3 – MPA and 4 – VPA. These variables were created with the `get_PAI_categories` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/get_PAI_categories.R), as described [here](#prep_data).
...
A Kappa coefficient of <0 is considered poor, .00–.20 slight, .21–.40 fair, .41–.60 moderate, .61–.80 substantial, and .81–1.00 almost perfect [[@landis77]](#refs).

Kappa statistic results for all accelerometer metrics of hip placement can be found on table below. For all metrics, VPA showed the lowest Kappa coefficient (classified as moderate). All other PAI showed Kappa coefficients classified as substantial or almost perfect.
...
```{r source_cp, echo=FALSE}
source(here("new_cutpoints.R"))
```
...
kappa_hip <- data.frame(
  ...
  kappa      = c(
    myround(kappa_hip_AC_SED[[5]], digits = 2), myround(kappa_hip_AC_MOD[[5]], digits = 2), myround(kappa_hip_AC_VIG[[5]], digits = 2), myround(kappa_hip_AC_glob[[5]], digits = 2), myround(kappa_hip_MAD_SED[[5]], digits = 2), myround(kappa_hip_MAD_MOD[[5]], digits = 2), myround(kappa_hip_MAD_VIG[[5]], digits = 2), myround(kappa_hip_MAD_glob[[5]], digits = 2), myround(kappa_hip_ENMO_SED[[5]], digits = 2), myround(kappa_hip_ENMO_MOD[[5]], digits = 2), myround(kappa_hip_ENMO_VIG[[5]], digits = 2), myround(kappa_hip_ENMO_glob[[5]], digits = 2)
  ),
  weight     = c(rep("unweighted", 3), "quadratic")
)
kable(kappa_hip, col.names = c("Accelerometer Metric", "Intensity Category", "Kappa", "Weight"))

```

Kappa statistic results for all accelerometer metrics at back placement can be found on table below. Similar to hip placement for all metrics, VPA showed the lowest Kappa coefficient (classified as moderate). SA, LPA and MPA showed Kappa coefficients classified as substantial or almost perfect.
...
kappa_back <- data.frame(
  ...
  intens_cat = c("Sedentary", "Moderate", "Vigorous", "Global"),
  kappa      = c(
    myround(kappa_back_AC_SED[[5]], digits = 2), myround(kappa_back_AC_MOD[[5]], digits = 2), myround(kappa_back_AC_VIG[[5]], digits = 2), myround(kappa_back_AC_glob[[5]], digits = 2), myround(kappa_back_MAD_SED[[5]], digits = 2), myround(kappa_back_MAD_MOD[[5]], digits = 2), myround(kappa_back_MAD_VIG[[5]], digits = 2), myround(kappa_back_MAD_glob[[5]], digits = 2), myround(kappa_back_ENMO_SED[[5]], digits = 2), myround(kappa_back_ENMO_MOD[[5]], digits = 2), myround(kappa_back_ENMO_VIG[[5]], digits = 2), myround(kappa_back_ENMO_glob[[5]], digits = 2)
  ),
  weight     = c(rep("unweighted", 3), "quadratic")
)
kable(kappa_back, col.names = c("Accelerometer Metric", "Intensity Category", "Kappa", "Weight"))

```

## Percent agreement

Percent agreement from global classification was also calculated. Hence, SA, LPA, MPA and VPA classification created from cut-points in each accelerometer metric from hip and back placement were recoded to create binary variables (0 or 1). The activity correctly identified was coded as 1 and the opposite as 0, taking as reference the classification obtained by indirect calorimetry. Percent agreement was calculated by `percent_agreement` [function](https://github.com/verasls/EE_PAI_ACC_obese/blob/master/R/percent_agreement.R) through the sum of activities correctly identified divided by the sum of all activities and then multiplied by 100.
...
kable(perc_agree, col.names = c("Accelerometer Placement", "Accelerometer Metric", "Percent Agreement"))
```
...
Prediction accuracy comparison among accelerometer metrics is shown below. In both accelerometer placements, no differences in prediction accuracy were observed among accelerometer metrics (*p*>0.05).
...
As happened with prediction accuracy comparison, new data frames needed to be built with classification errors in a single variable and grouped in another variable by accelerometer placement and metric.

Classification agreement comparison among accelerometer metrics is shown below. For hip accelerometer placement, no differences of classification agreement among accelerometer metrics were observed (*p*>0.05). For back accelerometer placement, although Kruskal-Wallis test *p* value was lower than 0.05, when *post hoc* tests were applied, no differences in classification agreement between each pair of accelerometer metrics were observed (*p*>0.05).
